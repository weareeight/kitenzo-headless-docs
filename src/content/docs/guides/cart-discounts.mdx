...
`POST /configure` returns a [`cart` object](/api/endpoints/#configure-a-bundle) for native bundles with both payloads pre-encoded: `line_items` (each carrying `_bundle_data`) and `attributes._bundles`. Write them to the cart as-is — merging `_bundles` with any existing value — and the transform does the rest. This is how the [TapCart integration](/guides/tapcart/) works.
